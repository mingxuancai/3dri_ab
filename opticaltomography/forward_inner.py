...
class Multislice:
    ...
    def _genSphericalWave(self, fx_source, fy_source, fz_depth, prop_distance=0, wl_coef=1.0, device='cpu'):
        ...
        source_xy = 1.0 * torch.exp(1.0j * 2.0 * np.pi / self.wavelength * r)/r
    ...
    def predictSLMpattern(self, fx, fy, fz, obj, focal_length, device='cuda:3'):
        """
        Simulate the wave propagation through the reconstructed sample.
        Get the corresponding SLM pattern for phase conjugation.
        focal_length: should be negtive for back propagation
    ...
    def predictSLM_dif(self, fx, fy, fz, obj, gt, focal_length, dif, device='cpu'):
        """
        Simulate the wave propagation through the reconstructed sample.
        Get the corresponding SLM pattern for phase conjugation.
        focal_length: should be negtive for back propagation
        ...
        SLM_field = self._backwardMeasure_amp(fields, fx, fy, fz-dif, gt, device=device)
        SLM_field = self._propagationAngular(SLM_field, dif, device=device)
        SLM_field = torch.abs(SLM_field*torch.conj(SLM_field))
